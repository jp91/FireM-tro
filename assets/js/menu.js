...
menu.show_menu = function(container) {
    ...
    if ( (ws_name=="") || (coverage=="")) {
        ...
        $.getJSON('./params.json', function(params) {
            ...
            str = "";
            str+="<section class=\"menu-section\"><h3 class=\"menu-section-title\">Menu</h3><ul class=\"menu-section-list\">";
            for (var i in menu.page_list){
                url=menu.page_list[i].name+".html?ws_name="+ws_name+"&coverage="+coverage;
                str+="<li>"+"<a href='"+url+"'>" +menu.page_list[i].title + "</a></li>";
            }
            str+="</ul></section>";
            //on affiche la liste des coverage
            str+="<section class=\"menu-section\"><h3 class=\"menu-section-title\">Couverture</h3><div id='coverage_list'><select id='coverage'><option>"+coverage+"</option></select></div></section>";
            document.getElementById(container).innerHTML=str;
...
menu.show_menu2 = function(container) {
    t=extractUrlParams();
    ws_name = (t["ws_name"])?t["ws_name"]:"";
    coverage = (t["coverage"])?t["coverage"]:"";
    token = (t["token"])?t["token"]:"";
